...
builder.Services.Configure<CredentialEncryptionOptions>(
    builder.Configuration.GetSection(CredentialEncryptionOptions.CredentialEncryption));
...
builder.Services.AddControllersWithViews(options =>
{
    ...
    options.Filters.Add<MailArchiver.Attributes.StrictReadOnlyArchiveFilter>();
})
    .AddViewLocalization();
...
// Fail before migrations or background jobs run if the mounted encryption secret
// is not valid. This avoids ever accepting a plaintext mailbox password.
_ = app.Services.GetRequiredService<ICredentialEncryptionService>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        {
            var derivedName = MailAccountNamePolicy.Derive(account.EmailAddress);
            if (account.Name != derivedName || !string.IsNullOrEmpty(account.GroupName))
            {
                account.Name = derivedName;
        ...
        if (normalizedCount > 0)
        {
            ...
            var normalizationLogger = services.GetRequiredService<ILogger<Program>>();
            normalizationLogger.LogInformation(
                "Normalized names and removed departments for {AccountCount} IMAP accounts",
                normalizedCount);
        }
        ...
        if (!string.IsNullOrWhiteSpace(bootstrapUsername))
        {
                     ...
                     !userService.VerifyPassword(authOptions.Password, adminUser.PasswordHash ?? string.Empty))
            {
                ...
                // the single administrator account. This keeps password
                // changes reproducible without exposing a user-management UI.
                adminUser.PasswordHash = userService.HashPassword(authOptions.Password);
                await userService.UpdateUserAsync(adminUser);
                var userLogger = services.GetRequiredService<ILogger<Program>>();
                userLogger.LogInformation("Admin password synchronized from Authentication configuration");
            }
        }
        ...
        await ApplyDeletionPolicyAsync(context, deletionPolicy, initLogger, isLocalApp);
    }
    catch (Exception ex)
    {
        var logger = services.GetRequiredService<ILogger<Program>>();
...
/// </summary>
static async Task ApplyDeletionPolicyAsync(MailArchiverDbContext context, DeletionPolicyOptions policy, ILogger<Program> logger, bool isLocalApp)
{
...
// This appliance is intentionally Chinese-only. Ignore a previously stored
// language cookie or the browser's Accept-Language header so every screen is
// rendered consistently in Simplified Chinese.
