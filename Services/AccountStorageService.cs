...
namespace MailArchiver.Services
{
    public class AccountStorageService : IAccountStorageService
    {
        ...
        private async Task<(long mailBytes, long attachmentBytes)> CalculateAccountStorageAsync(int mailAccountId)
        {
            if (IsLocalApp())
            {
                var emails = await _context.ArchivedEmails
                    .Where(e => e.MailAccountId == mailAccountId)
                    .Select(e => new
                    {
                        e.Subject,
                        e.From,
                ...
                var mailBytes = emails.Sum(email =>
                    ByteLength(email.Subject) + ByteLength(email.From) + ByteLength(email.To) +
                    ByteLength(email.Cc) + ByteLength(email.Bcc) + ByteLength(email.Body) +
                    ByteLength(email.HtmlBody) + ByteLength(email.RawHeaders) +
                    (email.OriginalBodyText?.LongLength ?? 0) + (email.OriginalBodyHtml?.LongLength ?? 0));
                var attachmentBytes = await _context.EmailAttachments
                    .Where(a => a.ArchivedEmail.MailAccountId == mailAccountId)
        ...
        private async Task UpsertCacheAsync(int mailAccountId, long mailBytes, long attachmentBytes, long totalBytes)
        {
            if (IsLocalApp())
            {
                var cache = await _context.AccountStorageCaches.FindAsync(mailAccountId);
                if (cache == null)
                {
                    cache = new AccountStorageCache { MailAccountId = mailAccountId };
                    _context.AccountStorageCaches.Add(cache);
                }

                cache.MailBytes = mailBytes;
                cache.AttachmentBytes = attachmentBytes;
                cache.TotalBytes = totalBytes;
        ...
        private async Task EnsureBackfillStateDoneAsync(int mailAccountId)
        {
            if (IsLocalApp())
            {
                var state = await _context.AccountStorageBackfillStates.FindAsync(mailAccountId);
                if (state == null)
                {
                    state = new AccountStorageBackfillState { MailAccountId = mailAccountId };
                    _context.AccountStorageBackfillStates.Add(state);
                }
