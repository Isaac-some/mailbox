...
namespace MailArchiver.Services.Core
{
    ...
    public class EmailCoreService
    {
            ...
            int? allowedUserId = null)
        {
            ...
            // delivered message with an old header SentDate is still visible.
            var rangeDateColumn = useReceivedDateForRange ? "ReceivedDate" : "SentDate";
            if (fromDate.HasValue)
            {
                whereConditions.Add($@"""{rangeDateColumn}"" >= @param{paramCounter}");
            ...
            {
                whereConditions.Add($@"LOWER(""FolderName"") = LOWER(@param{paramCounter})");
            ...
            int? allowedUserId = null)
        {
            ...
            if (fromDate.HasValue)
                baseQuery = useReceivedDateForRange
                    ? baseQuery.Where(e => e.ReceivedDate >= fromDate.Value)
                    : baseQuery.Where(e => e.SentDate >= fromDate.Value);

            if (toDate.HasValue)
                baseQuery = useReceivedDateForRange
                    ? baseQuery.Where(e => e.ReceivedDate <= toDate.Value.AddDays(1).AddSeconds(-1))
                    : baseQuery.Where(e => e.SentDate <= toDate.Value.AddDays(1).AddSeconds(-1));
