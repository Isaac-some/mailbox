...
namespace MailArchiver.Services.Providers
{
    ...
    /// IMAP service classes: ImapMailSyncService, ImapConnectionFactory,
    ...
    public class ImapEmailService : IProviderEmailService
    {
        ...
        public Task SyncMailAccountAsync(MailAccount account, string? jobId = null, MailSyncRequestOptions? options = null)
            => _syncService.SyncMailAccountAsync(account, jobId, options);
        ...
        {
            _logger.LogWarning("Blocked IMAP restore request in strict read-only mode");
            return false;
        }
        ...
        public async Task<(int Successful, int Failed)> RestoreMultipleEmailsWithProgressAsync(
            ...
            CancellationToken cancellationToken)
        {
            _logger.LogWarning("Blocked IMAP batch restore request in strict read-only mode");
