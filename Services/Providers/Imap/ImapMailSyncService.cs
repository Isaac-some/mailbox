    ...
    public class ImapMailSyncService
    {
        ...
        public async Task SyncMailAccountAsync(MailAccount account, string? jobId = null)
        {
            ...
            try
            {
                ...
                // This product is a strict archive: it never deletes remote messages
                // or applies local-retention cleanup during a sync.
                        ...
                        catch (Exception fallbackEx)
                        {
                            ...
                            if (_mailSyncOptions.LookbackDays > 0)
                            {
                                ...
                                _logger.LogInformation("Date filter kept {Count} messages newer than {Cutoff} in folder {FolderName}",
                                    uids.Count, lookbackCutoff, folder.FullName);
                            }
        ...
        private async Task<IList<UniqueId>> FilterUidsByDateAsync(
            ...
            DateTime cutoff)
        {
            if (uids.Count == 0 || cutoff == DateTime.MinValue)
                return uids;
        ...
        {
            _logger.LogWarning("Remote deletion was blocked by the read-only archive policy for account {AccountName}", account.Name);
            return 0;

#if false // Keep the former upstream implementation out of the compiled application.
