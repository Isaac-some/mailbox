...
namespace MailArchiver.Services
{
    public class MailSyncBackgroundService : BackgroundService
    {
        ...
        private readonly IMailSyncTrigger _syncTrigger;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            if (!_configuration.GetValue<bool>("MailSync:Enabled", false))
            {
                _logger.LogInformation("Automatic sync is disabled; accounts only sync when explicitly started by the user.");
                return;
            }

            // IntervalSeconds enables near-real-time quick syncs without changing the
            // existing per-account minute setting. Explicit per-account values win.
            var defaultSyncIntervalSeconds = _configuration.GetValue<int?>("MailSync:IntervalSeconds");
            var legacyDefaultSyncIntervalMinutes = _configuration.GetValue<int>("MailSync:IntervalMinutes", 15);
            var pollInterval = MailSyncSchedulePolicy.ResolvePollInterval(
                _configuration.GetValue<int>("MailSync:PollIntervalSeconds", 5));
            var failureRetryInterval = MailSyncSchedulePolicy.ResolveFailureRetryInterval(
                _configuration.GetValue<int>("MailSync:FailureRetrySeconds", 300));
            ...
            var startupStaggerSeconds = Math.Max(
                0,
                _configuration.GetValue<int>("MailSync:StartupStaggerSeconds", 3600));

            _logger.LogInformation(
                "Automatic sync enabled. Default interval: {DefaultInterval}; poll cadence: {PollInterval}; failure retry: {FailureRetry}",
                MailSyncSchedulePolicy.ResolveSyncInterval(
                    null, defaultSyncIntervalSeconds, legacyDefaultSyncIntervalMinutes),
                pollInterval,
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    var dueAccounts = new List<(MailAccount Account, bool PerformFullSync, TimeSpan SyncInterval)>();
                    foreach (var account in accounts)
                    {
                        ...
                        {
                            nextRunUtc[account.Id] = MailSyncSchedulePolicy.ResolveInitialNextRun(
                                account.Id,
                                account.LastSync,
                                nowUtc,
                        ...
                        var effectiveSyncInterval = MailSyncSchedulePolicy.ResolveSyncInterval(
                            account.SyncIntervalMinutes,
                            defaultSyncIntervalSeconds,
                            legacyDefaultSyncIntervalMinutes);

                        // Schedule the next normal sync from "now".
                        // Completion/failure handling below replaces this provisional value.
                        nextRunUtc[account.Id] = nowUtc.Add(effectiveSyncInterval);
                        ...
                        dueAccounts.Add((account, performFullSync, effectiveSyncInterval));
                    }
                    ...
                    if (dueAccounts.Count > 0)
                    {
                        ...
                        await Parallel.ForEachAsync(dueAccounts, parallelOptions, async (item, ct) =>
                        {
                            ...
                            var effectiveSyncInterval = item.SyncInterval;
                            ...
                            try
                            {
                                ...
                                // Schedule from completion, rather than from start, so a slow
                                // sync does not immediately run again. If the provider returned
                                // without advancing LastSync (partial/rate-limited sync), back
                                // off instead of repeatedly scanning the same mailbox.
                                DateTime? persistedLastSync;
                                using (var watermarkScope = _serviceProvider.CreateScope())
                                {
                                    var watermarkContext = watermarkScope.ServiceProvider
                                        .GetRequiredService<MailArchiverDbContext>();
                                    persistedLastSync = await watermarkContext.MailAccounts
                                        .AsNoTracking()
                                        .Where(a => a.Id == account.Id)
                                        .Select(a => (DateTime?)a.LastSync)
                                        .FirstOrDefaultAsync(ct);
                                }

                                if (!persistedLastSync.HasValue || persistedLastSync.Value <= account.LastSync)
                                {
                                    nextRunUtc[account.Id] = DateTime.UtcNow.Add(failureRetryInterval);
                                    _logger.LogWarning(
                                        "Sync watermark did not advance for account {AccountName}; retrying after {FailureRetry}",
                                        account.Name,
                                        failureRetryInterval);
                                }
                                else
                                {
                                    nextRunUtc[account.Id] = DateTime.UtcNow.Add(effectiveSyncInterval);
                                }
                ...
                _logger.LogInformation("Mail sync cycle completed. Waiting for next poll or account import.");
                try
                {
                    await _syncTrigger.WaitForNextPollAsync(pollInterval, stoppingToken);
                }
                catch (OperationCanceledException)
                {
